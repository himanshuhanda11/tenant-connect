...
BEGIN
  ...
  v_needs_conv := (
    p_assigned_agent IS NOT NULL
    OR p_unassigned_only
    OR (p_lead_statuses IS NOT NULL AND array_length(p_lead_statuses, 1) IS NOT NULL)
  );
  ...
  WITH matched AS (
    ...
    WHERE c.tenant_id = p_tenant_id
      ...
      -- Agent + Lead Status checked against the SAME conversation row (mirrors Inbox semantics)
      AND (
        NOT v_needs_conv OR EXISTS (
          SELECT 1 FROM public.conversations cv
          WHERE cv.tenant_id = p_tenant_id
            AND cv.contact_id = c.id
            AND (p_assigned_agent IS NULL OR cv.assigned_to = p_assigned_agent)
            AND (NOT p_unassigned_only OR cv.assigned_to IS NULL)
            AND (
              p_lead_statuses IS NULL
              OR array_length(p_lead_statuses, 1) IS NULL
              OR COALESCE(cv.crm_status::text, 'new') = ANY(p_lead_statuses)
            )
        )
      )
      -- Tags
      AND (
        p_include_tag_ids IS NULL OR array_length(p_include_tag_ids,1) IS NULL OR p_tag_match_all OR EXISTS (
          SELECT 1 FROM public.contact_tags ct WHERE ct.contact_id = c.id AND ct.tag_id = ANY(p_include_tag_ids)
        )
      )
      AND (
      ...
      AND (
        p_exclude_tag_ids IS NULL OR array_length(p_exclude_tag_ids,1) IS NULL OR NOT EXISTS (
          SELECT 1 FROM public.contact_tags ct WHERE ct.contact_id = c.id AND ct.tag_id = ANY(p_exclude_tag_ids)
        )
      )
      -- Attributes
      AND (
        v_attr_count = 0 OR (
          SELECT count(*) FROM (
            SELECT 1 FROM jsonb_array_elements(p_attributes) AS a
            WHERE EXISTS (
              SELECT 1 FROM public.contact_attributes ca
              WHERE ca.tenant_id = p_tenant_id AND ca.contact_id = c.id
                AND ca.key = (a->>'key') AND ca.value ILIKE '%' || (a->>'value') || '%'
            )
          ) hits
        ) = v_attr_count
      )
      ...
      AND (
        p_exclude_recent_days <= 0 OR NOT EXISTS (
          ...
          WHERE s.tenant_id = p_tenant_id AND s.contact_id = c.id
            AND s.last_message_at IS NOT NULL AND s.last_message_at >= v_recent_cutoff
        )
      )
  )
  SELECT
