CREATE OR REPLACE FUNCTION public.contacts_crm_search(p_tenant_id uuid, p_phone_number_id text DEFAULT NULL::text, p_lead_states text[] DEFAULT NULL::text[], p_is_unreplied boolean DEFAULT NULL::boolean, p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_date_to timestamp with time zone DEFAULT NULL::timestamp with time zone, p_assigned_to uuid DEFAULT NULL::uuid, p_claimed_by uuid DEFAULT NULL::uuid, p_last_replied_by uuid DEFAULT NULL::uuid, p_search text DEFAULT NULL::text, p_tag_ids uuid[] DEFAULT NULL::uuid[], p_tag_match_all boolean DEFAULT false, p_attributes jsonb DEFAULT NULL::jsonb, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_crm_statuses text[] DEFAULT NULL::text[])
 RETURNS TABLE(tenant_id uuid, phone_number_id text, contact_id uuid, open_conversation_id uuid, lead_state text, is_unreplied boolean, last_message_at timestamp with time zone, last_inbound_at timestamp with time zone, last_outbound_at timestamp with time zone, assigned_to uuid, assigned_at timestamp with time zone, claimed_by uuid, claimed_at timestamp with time zone, last_replied_by uuid, last_replied_at timestamp with time zone, contact_name text, wa_id text, first_name text, profile_picture_url text, tags jsonb, attributes jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      cis.lead_state,
      cis.is_unreplied,
      cis.last_message_at, cis.last_inbound_at, cis.last_outbound_at,
      cis.assigned_to, cis.assigned_at, cis.claimed_by, cis.claimed_at,
      cis.last_replied_by, cis.last_replied_at,
      c.name AS contact_name, c.wa_id, c.first_name, c.profile_picture_url,
      c.created_at AS contact_created_at
    FROM public.contacts c
    LEFT JOIN public.contact_inbox_summary cis
      ON cis.contact_id = c.id AND cis.tenant_id = c.tenant_id
    WHERE c.tenant_id = p_tenant_id
      ...
      AND (p_date_from IS NULL OR COALESCE(cis.last_message_at, c.created_at) >= p_date_from)
      AND (p_date_to IS NULL OR COALESCE(cis.last_message_at, c.created_at) <= p_date_to)
  ...
  ORDER BY COALESCE(af.last_message_at, af.contact_created_at) DESC NULLS LAST
