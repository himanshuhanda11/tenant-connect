CREATE OR REPLACE FUNCTION public.agent_performance_stats(p_tenant_id uuid, p_days integer DEFAULT 7)
 RETURNS TABLE(agent_id uuid, agent_name text, agent_email text, avatar_url text, is_online boolean, last_login_at timestamp with time zone, total_sessions integer, total_hours_worked numeric, avg_daily_hours numeric, chats_opened bigint, chats_replied bigint, chats_assigned bigint, leads_clicked bigint, conversations_claimed bigint, conversations_converted bigint, current_open_chats bigint, avg_response_minutes numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  response_times AS (
    SELECT
      c.assigned_to AS user_id,
      ROUND(AVG(EXTRACT(EPOCH FROM (c.first_response_at - c.created_at)) / 60)::numeric, 1) AS avg_resp_min
    FROM public.conversations c
    WHERE c.tenant_id = p_tenant_id
