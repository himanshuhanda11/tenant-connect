  ...
  try {
    ...
    if (!tenant_id || !phone_number_id || !to_wa_id) {
      return new Response(JSON.stringify({ 
        error: 'tenant_id, phone_number_id, and to_wa_id are required' 
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    // 3. Upsert contact by tenant_id + wa_id
    ...
    // 4. Upsert conversation by tenant_id + phone_id + contact_id
    ...
    // 6. Create pending message in database
    ...
    // 7. Call Meta Cloud API
    try {
      console.log('Sending to WhatsApp API:', phoneNumber.phone_number_id);
      ...
      // 8. Update message with wamid + status=sent + sent_at
