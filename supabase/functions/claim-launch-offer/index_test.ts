...
//
// Tests are skipped if SUPABASE_SERVICE_ROLE_KEY is not present.
...
async function cleanup(userId: string, tenantIds: string[]) {
  for (const id of tenantIds) await admin.from("tenants").delete().eq("id", id);
  await admin.auth.admin.deleteUser(userId);
}

Deno.test({
  name: "claim_launch_offer: enforces one paid trial per user across workspaces",
  ignore: !CAN_RUN,
  fn: async () => {
    ...
    try {
      ...
      const r1 = await c.rpc("claim_launch_offer", { _plan_id: "pro", _workspace_id: wsA.id });
      assertEquals(r1.error, null, `first claim error: ${r1.error?.message}`);
      const d1 = r1.data as ClaimResult;
      assert(d1?.ok === true, `expected ok:true, got ${JSON.stringify(d1)}`);
      assertEquals(d1.plan_id, "pro");

      const r2 = await c.rpc("claim_launch_offer", { _plan_id: "basic", _workspace_id: wsB.id });
      assertEquals(r2.error, null, `second claim error: ${r2.error?.message}`);
      ...
      const r3 = await c.rpc("claim_launch_offer", { _plan_id: "free", _workspace_id: wsB.id });
      assertEquals(r3.error, null, `free claim error: ${r3.error?.message}`);
...
Deno.test({
  name: "claim_launch_offer: rejects non-members of the workspace",
  ignore: !CAN_RUN,
  fn: async () => {
    ...
    try {
      ...
      assertEquals(succeeded, false, "stranger must not claim trial on someone else's workspace");
    } finally {
...
Deno.test({
  name: "claim_launch_offer: returns structured JSON (no 2xx/500 mismatch)",
  ignore: !CAN_RUN,
  fn: async () => {
    const user = await makeUser();
    const c = await userClient(user.email, user.password);
    ...
    try {
      ...
      const r = await c.rpc("claim_launch_offer", { _plan_id: "free", _workspace_id: ws.id });
      assertEquals(r.error, null, `RPC must not error: ${r.error?.message}`);
