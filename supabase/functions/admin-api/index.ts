  ...
  try {
    ...
    if (req.method === "GET" && path.startsWith("workspaces/") && !path.includes("/", 11)) {
      ...
      const [workspace, entitlements, members, phones, wsPhone, waba, ownerMember] = await Promise.all([
        ...
        sb.from("waba_accounts").select("id,waba_id,business_id,name,status,token_source,created_at,updated_at,onboarding_type,coexistence_enabled,coexistence_status,coexistence_eligibility,coexistence_error,coexistence_checked_at,is_on_biz_app,platform_type,contacts_sync_request_id,history_sync_request_id,contacts_sync_status,history_sync_status,history_sync_progress,history_sharing_enabled,last_smb_echo_at,disconnect_reason").eq("tenant_id", workspaceId).order("created_at", { ascending: false }).maybeSingle(),
    ...
    // POST /users/:id/send-reminder  { type: 'signup' | 'workspace' }
    if (req.method === "POST" && path.match(/^users\/[^/]+\/send-reminder$/)) {
      const userId = path.split("/")[1];
      const actor = await requirePlatformRole(req, ["super_admin", "support"]);
      const sb = adminClient();
      ...
      const { error } = await sb.functions.invoke("send-transactional-email", {
        body: {
          ...
          idempotencyKey: `manual-${type}-reminder-${userId}-${stamp}`,
          templateData,
        },
      });
      if (error) throw new Error(error.message || "Email send failed");
      await sb.from("signup_reminder_log").insert({
        user_id: userId, email: user.email, reminder_type: type, reminder_stage: `manual-${stamp}`,
      }).then(() => null, () => null);
      await logAction(sb, actor, "PLATFORM_REMINDER_SENT", {
        target_table: "auth.users", target_id: userId, note: `Manual ${type} reminder to ${user.email}`,
      });
